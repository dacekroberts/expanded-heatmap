"""Amsterdam step 1: GVB's metro and tram stations inside the gemeente.

    python pipeline/amsterdam/step1_stations.py

Reads the cache only; `fetch_sources.py` downloads.

WHAT MAKES THIS CITY'S STEP 1 DIFFERENT:

  * **The feed is national.** GVB's 21 rail routes are read out of OVapi's
    CC0 national GTFS; nothing else in it is touched.
  * **A line's stations are its REGULAR ROUTE** - the stops it serves on most
    days of the feed's window - because GVB runs up to 30 shapes per line
    here (short workings, weekend closures, works diversions) and neither the
  ...
  * Step 3's line geometry is written here as a small zip of GVB's shapes, so
...
from pipeline.amsterdam import config  # noqa: E402
...
def short(key):
    """'Amsterdam, Dam#0' -> 'Dam'; other towns keep their prefix."""
    name = key.split("#", 1)[0]
    return name.split(", ", 1)[1] if name.startswith("Amsterdam, ") else name


def places(stops, used_ids):
    """stop_id -> PLACE key. A GVB stop name is a street, not a place: the
...
def load_feed():
    ...
    if sorted(lines) != sorted(config.LINE_ORDER):
        sys.exit(f"GVB's rail lines changed.\n  feed  : {lines}\n"
                 f"  config: {sorted(config.LINE_ORDER)}\n"
...
def write_shapes(z, shape_ids):
    """GVB's chosen shapes only, as a one-file GTFS zip for step 3."""
    ...
    with zipfile.ZipFile(config.GVB_SHAPES_ZIP, "w", zipfile.ZIP_DEFLATED) as out:
...
def main():
    ...
    print(f"GVB rail in the national feed: {len(rail)} lines, {len(trips):,} trips, "
          f"{len(st):,} stop times")
    ...
    station_gates.verify_stations(
        city="Amsterdam", platforms=q, stations=by_name, crs_projected=config.CRS_PROJECTED,
    ...
    for n in sorted(names - inside):
        ...
        if len(hit) != 1:
            sys.exit(f"{n} is outside Amsterdam and in {len(hit)} recorded neighbour(s)")
    ...
    print(f"  {n_shapes} GVB shapes -> {config.GVB_SHAPES_ZIP.relative_to(config.ROOT)}")
