"""Step 3 - Render Toulouse's heatmap to a standalone HTML file.

All rendering lives in pipeline/map_common.py; this file supplies only what is
Toulouse-specific.

Input:  data/toulouse/processed/stations.csv
        data/toulouse/processed/businesses_clean.csv
        data/toulouse/raw/gtfs.zip
        data/toulouse/raw/city_boundary.geojson   (label anchoring)
Output: outputs/toulouse/heatmap.html

Run:  python pipeline/toulouse/step3_map.py

The shape-selection rule is Dublin's, as Marseille takes it: shapes in
descending trip order, keep any that reaches a stop the kept ones do not, stop
when the route's stops are covered. It costs nothing on a line with one
alignment and silently saves one that branches.

⚠ ONE LINE HERE IS AN AERIAL CABLE CAR. Téléo's `shapes.txt` geometry is a
straight span between towers rather than a street alignment, which is correct
and will look different from the others - it crosses the Garonne and the
Rangueil hillside in three hops. It is drawn from the feed's real geometry like
every other line; nothing special-cases it.
"""
import json
import sys
import zipfile
...
from pipeline.toulouse.config import (  # noqa: E402
...
def commune_geometry():
    """The commune of Toulouse, so each line's label goes at the tail of the
    stretch inside it.

    This matters more here than in Marseille, where every station was inside.
    T1 runs 12 stations past this boundary into Blagnac and Beauzelle, so
    without the focus its label would be anchored in another commune - on a
    stretch this map deliberately does not count."""
...
def main():
    ...
    render_heatmap(
        ...
        map_title="Toulouse Métro, Tramway and Téléo Business Density Heatmap",
        city_name="Toulouse",
        system_name="Métro, Tramway and Téléo",
        stations=stations,
        businesses=businesses,
        taxonomy_system=TAXONOMY_SYSTEM,
        lines=load_line_shapes(GTFS_ZIP, line_specs,
                               "Métro, Tramway and Téléo"),
