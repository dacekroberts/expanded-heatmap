...
Taichung's door plates.

  1. The register rows whose address is in Taichung and whose industry code is
     in a storefront division (pipeline/countries/taiwan.py; 487 online
     shopping excluded).
  ...
  3. The JOIN: the address parsed to district, street, lane, alley and number
     ...
     because Taichung's 2010 merger left one street name in several districts;
     then the parent number's plate (tier 2). Unmatched rows are left off.
...
def need(path, what, arg):
    if not path.exists():
        sys.exit(f"missing {path} ({what}).\nRun: python pipeline/taichung/fetch_sources.py {arg}")
    return path
...
def load_plates():
    ...
    plates, plates_nd = {}, collections.defaultdict(set)
    with need(config.DOORPLATE_CSV, "door plates", "doorplates").open(
        ...
        for r in rd:
            ...
            if not k:
                continue
            try:
                pt = (float(r[cols["lon"]]), float(r[cols["lat"]]))
            except ValueError:
                continue
            d = r[dcol].strip()
            plates.setdefault((d, *k), pt)
            plates_nd[k].add(d)
...
def main():
    ...
    rows = []
    for r in taiwan.register_rows(config.ADDRESS_PREFIXES, need(taiwan.REGISTER_ZIP, "tax register", "register")):
    ...
    print(f"  register rows in Taichung, storefront divisions: {len(df):,}  "
          f"{df.bucket.value_counts().to_dict()}")
    emit("storefront_rows", len(df))

    df["district"] = df.address.map(district_of)
