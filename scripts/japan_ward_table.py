    ...
    for city, (name, band, _) in STATUS.items():
        ...
        for w, v in order:
            ...
            if city == "tokyo":
                ...
                share = v["Food service"] / yb[w] if yb.get(w) else None
        ...
        if city == "tokyo":
            ...
            L += ["", "**Of the official count** is the list's fixed-premises food-service rows against 飲食店営業 in "
                  "Tokyo's statistical yearbook, table 19-8, for the latest fiscal year. The yearbook also counts "
                  "vehicle permits (about 2% of Shibuya's list), so a complete list reads a little under 100%. "
                  "✅ marks 90% or more. **The wards differ because each is its own publisher.** "
...
def main():
    ...
    text = render(measure(), tokyo_completeness(), yearbook())
    print(text)
