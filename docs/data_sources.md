...
| **Kyoto** — the City's permit lists on `data.city.kyoto.lg.jp`: 00414, 00541 (food) and 00530 (barber, beauty, laundry) (**candidate, not built**; read 2026-09-24 by the `licence-read` agent) | **CC BY 4.0**, applied by the portal's 京都市オープンデータ利用規約 (第3版) clause 1. All 340 resource pages declare it; there is no click-through, and the government standard terms are not adopted. **PERMITTED WITH CONDITIONS**: name the creator and the name 京都市オープンデータ, link, state the licence, say the data was processed. Must not imply endorsement or use the City's emblem. Fetch only from the portal: older copies on `www.city.kyoto.lg.jp` fall under that site's no-copying page. Details and a draft notice: `docs/build_briefs/kyoto.md` | 著作権者 **京都市**. The portal asks: state that 京都市オープンデータ was used |
