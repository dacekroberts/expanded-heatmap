...
| **Tokyo 総務局's COVID-era lists**: 徹底点検TOKYOサポート 認証店 (`soumu/130001_tokyo_tenken.csv`) and 感染防止徹底宣言ステッカー (`soumu/bousai/130001_tokyo_tenpomap.csv`), frozen May 2023 | ⚠️ **Ambiguous in a way that matters: NOT USED pending the owner** (found 2026-09-24; not yet given a full `licence-read`). The catalogue declares CC BY 4.0, but each dataset's note says the shops agreed to share their data for COVID measures and asks users not to use it for any other purpose (「それ以外の目的に用いないようにお願いします」). CC BY cannot carry such a condition, but the publisher's stated wish and the shops' consent are both limited to that purpose, and this project honours a publisher's request rather than arguing it | They would lift Chūō, Kōtō and Minato to about 50–60% of the official count |
