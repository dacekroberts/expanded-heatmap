...
| **Japan** — the candidate cities' permit lists, MHLW's open data, MLIT's address, rail and boundary layers (**candidates, not built**) | Mostly **PERMITTED WITH CONDITIONS** (CC BY 4.0, CC BY 2.1 JP, PDL 1.0). Sendai's lists need permission. N03's boundaries may never be drawn (Survey Act). **Four Tokyo wards are not yet read.** One source per row in the Japan section below | Per source, below |
...
Act condition below. The four Tokyo wards are still listed as unread rather
than borrowed from a neighbour's verdict.
...
| **Tokyo: Shibuya** (the ward's ArcGIS open-data site), **Taitō** and **Setagaya** (the wards' own sites), **Meguro** (BODIK) | ⚠️ **NOT YET READ.** The owner accepted the three off-domain hosts as the wards' own channels on 2026-09-24, but no one has read their terms. Four reads are due before Tokyo's build, which comes last | — |
