...
**22. Comune di Milano — required, and NOT YET DISPLAYED because Milan is not
built into the deployed app.**

⚠️ **THIS NUMBER COLLIDES WITH DUBLIN'S, which is also 22 on the
`dublin-build` branch.** Both cities were built the same day on separate
branches and each took the next free number on its own. Dublin was built
first, so **at merge Dublin keeps 22 and this becomes 23** — renumber this
item and the three `notice **22**` citations in Milan's provenance rows
together. `check_provenance.py` catches duplicate and non-contiguous notice
numbers (it found two item 8s and two item 15s once), so the merge will say
so if this is forgotten — but it is cheaper to do deliberately. Recorded at build time so the obligation exists
