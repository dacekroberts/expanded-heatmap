...
| Oslo | **Brønnøysundregistrene's Enhetsregisteret — the SUB-UNITS (`underenheter`)**, each a place of business with its own `beliggenhetsadresse`, the PHYSICAL location kept apart from the company's registered `forretningsadresse` — joined on `overordnetEnhet` to the MAIN UNITS (`enheter`) for one field, the legal form | All three buckets via **SN2025** (NACE Rev. 2.1 — the register switched from SN2007, which the brief assumed), `pipeline/taxonomies/norway_sn2025.py`, keyed at the 5-digit level. **10,718 storefronts** built 2026-09-24 from 13,458 sub-units in divisions 47/56/96 | `https://data.brreg.no/enhetsregisteret/api/underenheter/lastned/csv` (61.4 MB gzipped) and `https://data.brreg.no/enhetsregisteret/api/enheter/lastned/csv` (154.7 MB), keyless, cached NATIONALLY at `data/norway/raw/` as France's parquets are | `beliggenhetsadresse.kommunenummer == "0301"` — ⚠️ **never the paged API's `?kommunenummer=`**, which does not constrain the location address (its misses included Bergen, Copenhagen, Paris and Malmö). ⚠️ **The CSV carries `epostadresse`, `telefon` and `mobil`**; step 2 reads by `usecols` and asserts none arrives. **The sole-trader guard lives on the PARENT**: a sub-unit's own form is BEDR/AAFY on 100% of rows and never ENK, so every premises is joined to its main unit and an `ENK`'s (31.3%) name is replaced by its address. Parents bankrupt or being wound up dropped (260). Structural exclusions by label 1,314; catch-all `96.990` (811) excluded on Oslo's numbers. Licence **NLOD** — notice **27** | 2026-09-24 |
| Oslo | **Kartverket's Matrikkelen – Adresse, kommune 0301** (106,254 addresses, 106,136 street addresses) | Not businesses — **JOINED** to the above for coordinates, on street name, number, letter and postcode: **96.8%** placed (91.1% exact, 5.5% letter-agnostic, the rest a spelling or postcode fallback that accepts only a UNIQUE match). This replaced the brief's plan of ~13,000 calls to Kartverket's search API, and with it the `fuzzy=true` trap | `https://nedlasting.geonorge.no/geonorge/Basisdata/MatrikkelenAdresse/CSV/Basisdata_0301_Oslo_4258_MatrikkelenAdresse_CSV.zip` (5.7 MB, keyless). The per-kommune template is `https://nedlasting.geonorge.no/geonorge/Basisdata/MatrikkelenAdresse/CSV/Basisdata_{code}_{name}_4258_MatrikkelenAdresse_CSV.zip` | none — the file is the kommune. EPSG 4258 (ETRS89), read as lat/lon. The 3.2% unmatched lean to companies (13.2% sole traders vs 28.1%), mostly a building name or a c/o on the address line. Licence **CC BY 4.0** — notice **28** | 2026-09-24 |
| Copenhagen | **CVR, Det Centrale Virksomhedsregister — the PRODUCTION UNITS (`Produktionsenhed`, P-enheder)**, each a place of business with its own `beliggenhedsadresse`, distinct from any `postadresse` (2,800,921 against 928 nationally). **SIX national entity files joined on `CVREnhedsId`** — `Produktionsenhed` (the spine: `id`, `pNummer`, the parent's CVR number), `Adressering` (location), `Branche` (DB25 code AND label), `Navn` (trade name), `Virksomhed` and `Virksomhedsform` (the parent's legal form, for the privacy guard) | All three buckets via **DB25** (NACE Rev. 2.1 — the brief assumed DB07), `pipeline/taxonomies/denmark_db25.py`, keyed at 6 digits. **14,978 storefronts** built 2026-09-24 from 17,173 production units in divisions 47/56/96 in Kobenhavn (101) and Frederiksberg (147) | Datafordeler file download, `https://api.datafordeler.dk/FileDownloads/v1.0/GetFile?Filename=<name>&apiKey=<key>`, names from `https://api.datafordeler.dk/FileDownloads/v2.0/GetAvailableFileDownloads` — **the owner's free API key, read from `DATAFORDELER_API_KEY` and never printed** (it travels in the URL). Generation **505** (built the night of 18-19 September 2026), cached NATIONALLY at `data/denmark/raw/cvr/`, 2.8 GB for the six | `AdresseringAnvendelse == "beliggenhedsadresse"` and CVR's own `CVRAdresse_kommunekode` in {101, 147} — never a polygon. ⚠️ **`pNummer` is NOT the join key** — it appears in no other file. ⚠️ **Every file is versioned**: a row counts only while `virkningTil` is empty, even in a "current" download. ⚠️ **`sekvens` 0 is the hovedbranche**; other values count a premises more than once. ⚠️ **Weekly generation, 7-day retention**: `fetch_sources.py` refuses a CVR set that mixes generations. ⚠️ **`coNavn` (c/o — a person on 25% of storefront rows) is never loaded**, and step 2 asserts it. **The privacy guard is the parent's legal form**: Enkeltmandsvirksomhed, PMV and Interessentskab (owner's call) show the address, as does any name with the sole-trader marker `v/`. Structural exclusions 1,326; catch-all `969900` (613) excluded. Licence **CC BY 4.0** — notice **30** | 2026-09-24 |
| Copenhagen | **DAR, Danmarks Adresseregister** (Klimadatastyrelsen) via Datafordeler — `Adresse` and `Husnummer` nationally, `Adressepunkt` per kommune (0101, 0147) | Not businesses — **JOINED** to the above for coordinates: CVR's `Adressering.Adresse` is a DAR **Adresse** id (199 of 200 sampled against DAWA before it closed), so the chain is Adresse → `husnummer` → Husnummer → `adgangspunkt` → Adressepunkt `position`. **98.3%** placed; of 11,512 distinct ids, **one** did not resolve, and the 255 unplaced rows carry no id at all | The same Datafordeler endpoints and key. Generation **761** (18 September 2026). `Adresse` 1.5 GB and `Husnummer` 1.7 GB unzipped are NATIONAL because Datafordeler lists no per-kommune file for them; `Adressepunkt` is listed per kommune (0101 10.3 MB, 0147 1.2 MB zipped) | ⚠️ **NOT DAWA**: `api.dataforsyningen.dk` closes 1 October 2026 at 10:00 (Klimadatastyrelsen), which is why the brief's keyless leg was replaced. ⚠️ **DAR is bitemporal even in a "current" file** — a row counts while `registreringTil` AND `virkningTil` are empty. ⚠️ **Positions are EPSG:25832 (zone 32) nationally**, Copenhagen included, though the city's own zone is 33; converted on read. Licence **CC BY 4.0**, credit Klimadatastyrelsen — notice **31** | 2026-09-24 |
| Prague | **ROS02, *Registr osob – aktivní provozovny*** (Digitální a informační agentura) — every active ESTABLISHMENT (`ICP`) with its owner's `ICO` and the RUIAN address code of WHERE IT TRADES (`PKODADM`). **This is the premises register**, and the reason Prague resumed after the Statistical Office's RES, which records a subject's registered SEAT, paused it | Location only; the activity comes from RES below. **25,275 storefronts** built 2026-09-24 from 84,192 establishments active in obec 554782 | `https://www.szrcr.cz/images/dokumenty/ROS/Otevrena%20data/ros02_data.csv` (58,507,315 bytes, monthly, snapshot `DATPLAT` 2026-08-31), keyless, cached NATIONALLY at `data/czechia/raw/` | Rows repeat: dedupe on `ICP`. **Active is judged against the file's own `DATPLAT`, never today**, so a drift check next month gives this month's answer. 6.6% of active establishments nationally carry no `PKODADM`. ⛔ **Not RŽP via ARES**, though it carries an establishment type: not open data, and ÚOOÚ fined a site in 2019 for republishing sole traders' trade data from it (UOOU-10201/18-31). Licence: declared open data, no personal data, no database right — **PERMITTED, nothing to display** | 2026-09-24 |
| Prague | **RES, *Registr ekonomických subjektů*** (Český statistický úřad) — joined by `ICO` for the OWNER's activity, legal form and registered name | All three buckets via **CZ-NACE 2025** (`NACE2025`, NACE Rev. 2.1 — NOT the older `NACE` column the brief measured), `pipeline/taxonomies/czech_nace2025.py`, matched on PREFIX because RES stores codes at ragged depth. ⚠️ **Every establishment inherits its owner's single activity** | `https://opendata.czso.cz/data/od_org03/res_data.csv` (543,309,993 bytes, dated 2026-09-17), keyless, cached nationally; labels from ČSÚ's CZ-NACE 2025 codebooks 6101–6105 (`https://apl2.czso.cz/iSMS/do_cis_export?kodcis=<n>&typdat=0&cisjaz=203&format=2&separator=%2C`) | Owner found for 99.97%; 88 owners ended. **The privacy guard is `FORMA`** (codebook 56): natural persons 101/105/107/424/425 and the v.o.s. partnership 111 show the ADDRESS, and a natural person's establishment at their own registered seat (`PKODADM == KODADM`) is EXCLUDED (1,500) - owner's calls. `KATPO` is NOT read: `000` means *not stated*. Structural exclusions 550; no catch-all excluded (`96990` has 3 rows). Licence **CC BY 4.0** with ČSÚ's data conditions — notice **32** | 2026-09-24 |
| Prague | **RUIAN address export, obec 554782** (ČÚZK) | Not businesses — **JOINED** on the RUIAN address code for the point and the street-address label: **99.99%** placed (2 of 25,277 at the 6 addresses with no coordinates) | Resolved each run through ČÚZK's INSPIRE ATOM service `https://atom.cuzk.gov.cz/get.ashx?theme=RUIAN-CSV-ADR-OB&spatial_dataset_identifier_code=CZ-00025712-CUZK_RUIAN-CSV-ADR-OB_554782`, which named `20260831_OB_554782_ADR.csv.zip` (3,395,881 bytes) | ⚠️ **EPSG:5513 with the published (X, Y)**; the other axis orders land in Germany or the Arctic, so step 2 re-checks Prague Castle (code 21690278) on every run. ⚠️ An address with no coordinates transforms to INFINITY, not NaN, and is made missing before any bounding-box test. cp1250, `;`. The ATOM route sidesteps the VDP application's ban on automated extraction. Licence **CC BY 4.0**, credit "ČÚZK, <year>" — notice **33** | 2026-09-24 |
| Amsterdam | **Hospitality operating permits, *horeca exploitatievergunningen*** (Gemeente Amsterdam) — the city's LIVE register: every permit granted, expired ones pruned by the city within about three months. A named business (`zaaknaam`, the name on the permit) with a point, on the city's own DSO API `https://api.data.amsterdam.nl/v1` | **Food service only**: 4,092 permits -> **3,605 kept** by the owner's mapping of `zaakCategorie` (restaurants, cafés, alcohol-free, fast food, eethuis, coffeeshops, nightclubs; `Onbekend` sorted by its `zaakSpecificatie`) -> 3,583 placed. Out: `Additionele horeca` (canteens and foyers inside non-commercial venues), cultural venues, members' societies, hotels, hall hire. 128 permits past their end date and still `Verleend` are KEPT (renewals in progress; measured) | `https://api.data.amsterdam.nl/v1/horeca/exploitatievergunning/` — HAL JSON, `_pageSize` 1,000 and `_links.next`, `Accept: */*` or the root refuses; keyless today, an API key to become MANDATORY on an unset date (registering is the owner's act) | None at download. 96.7% carry an RD New point; the 136 without carry no postcode either, so the fetch script looks each up in the BAG by street and number and step 2 places 92 (exact 77, letter/toevoeging swapped 8, toevoeging dropped within one building 7), 22 unplaceable. Licence **SILENT** (`Licentie: -`, no `license` key in the live schema; a retired 2022 catalogue said CC BY) — **displayed as CC BY 4.0 on the owner's choice, which satisfies both readings**: credit Gemeente Amsterdam, link the licence, state the filtering; a notice is REQUIRED, wording drafted 2026-09-24 for the owner | 2026-09-24 |
| Amsterdam | **BAG units whose use class is *winkelfunctie*** (Kadaster's Basisregistratie Adressen en Gebouwen, via the city's copy on the same API) — what a unit is FOR, not what is in it: **no name, no activity** | **Shops and services, one category** — the BAG cannot split retail from personal services: 11,607 shop-class units -> 10,898 in use -> **10,141** after the 757 also registered as a DWELLING are left off (owner's call) -> 9,655 after 486 de-duplicated against a permit at the same address. About 5% of Amsterdam's shop units are empty (the city's `bbga`, source Locatus, 1 January 2026: 660 of 14,314) and cannot be filtered per unit - disclosed, not filtered | `https://api.data.amsterdam.nl/v1/bag/verblijfsobjecten/?gebruiksdoel.omschrijving=winkelfunctie` (the filter is real: a nonsense value returns 0); each unit's address from `https://api.data.amsterdam.nl/v1/bag/nummeraanduidingen/` and street from `https://api.data.amsterdam.nl/v1/bag/openbareruimtes/`, both by `identificatie[in]` batches of 100 | Status `Verblijfsobject in gebruik`. **Only national BAG fields are used** - the city's own BAG-plus additions carry no licence. Licence **Public Domain Mark** (Kadaster; PDOK: no conditions) — **PERMITTED, nothing to display** | 2026-09-24 |
| Rome | **SUAP, *Elenco delle attività produttive*** (Roma Capitale's one-stop shop for productive activities) — every authorised PREMISES, keyed on (`STRUTTURA_GESTIONE`, `NUMERO_ESERCIZIO`), with the city's own street code, civic number and suffix and the type of authorisation. **No name column and no coordinates** | All three buckets by `DESCRIZIONE`, and inside the workshop catch-all `Laboratorio Artigianale e non` (37,832 rows, 22%) by `SPECIALIZZAZIONE` - blank on 19,963 of them, dropped - on NAICS 812 and Milan's artisan-food precedent (`pipeline/taxonomies/rome_suap.py`). 168,255 rows -> 155,448 establishments -> **103,375** storefronts (Retail 63,559, Food 25,826, Personal 13,990; 3,110 with two authorisations take the more specific) -> 98,897 placed | `https://dati.comune.roma.it/catalog/it/dataset/94848d96-0197-486b-b143-485f00285928/resource/3fc5fd67-6ed5-428a-88c4-cf5245f9ec0f/download/opendata_suap_luglio_2025.csv` (25,646,982 bytes) — **July 2025 is the newest file**; no 2026 dataset exists | ⚠️ **Not valid CSV**: no quoting, and `SPECIALIZZAZIONE` (12th of 14) carries commas on 30,075 rows, a few street names too; `pipeline/rome/suap.py` reads it by the fixed shape and validates every row. Some values double-encoded UTF-8 (`AttivitÃ`), repaired only where the round trip succeeds. `DATA_INIZIO` is an Excel serial date. ⚠️ **No closure date** - see the food control in `DECISIONS.md` (2.67x OSM; the OLDEST registrations match OSM best, so no age cut-off). Licence **CC BY 4.0** (the portal's *Licenze* page; resource `license_type` `A21_CCBY40`) — credit Roma Capitale, link, state the modification; a notice is REQUIRED, wording drafted 2026-09-24 for the owner | 2026-09-24 |
| Rome | **ANNCSU, *Archivio Nazionale dei Numeri Civici delle Strade Urbane*** (Agenzia delle Entrate and ISTAT), Lazio address list | Not businesses — **JOINED** on the city's street code (`CODICE_COMUNALE` = SUAP `CODICE_VIA`) + civic + suffix: 92.2% exact, 3.5% civic with the suffix dropped (only where every such civic lies within 60 m), **95.7% at civic level**; 2.7% not found, 1.6% with no civic number; worst municipio IX at 90.6%. **A street's centroid is never used** | `https://anncsu.open.agenziaentrate.gov.it/age-inspire/opendata/anncsu/getds.php?INDIR_LAZI` -> `INDIR_LAZI_20260915.csv` in a 24,574,080-byte zip; 516,337 civic numbers for ISTAT 058091, 516,331 with a point | ⚠️ **Refuses HEAD (403), answers GET.** ⚠️ **WGS84 longitude / latitude with DECIMAL COMMAS.** ⚠️ Fill varies by comune - the Lazio file's first comune has neither street code nor coordinates; check any other Italian city's own rows. Licence **CC BY 4.0** (dati.gov.it; the EU HVD regulation requires CC BY 4.0 or looser for addresses) — credit Agenzia delle Entrate and ISTAT, link, say the coordinates place other data; a notice is REQUIRED, wording drafted 2026-09-24 | 2026-09-24 |
| São Paulo | **IBGE CNEFE 2022** - the census's walk of every block: one row per use-type per address, the enumerator's description of each establishment (`DSC_ESTABELECIMENTO`) and point; classified by `pipeline/taxonomies/brazil_cnefe.py` (free text, rules version 2) | All three buckets: **219,578** storefronts placed; unreadable descriptions dropped, never guessed; at an address that also holds a dwelling the pin shows its category only | `https://ftp.ibge.gov.br/Cadastro_Nacional_de_Enderecos_para_Fins_Estatisticos/Censo_Demografico_2022/Arquivos_CNEFE/CSV/Municipio/35_SP/3550308_SAO_PAULO.zip` | none - the whole município file; `COD_ESPECIE` 6 read, 1-2 for the dwelling test, coordinate levels 1-4 kept | 2026-09-24 |
| Rio de Janeiro | **IBGE CNEFE 2022** - the census's walk of every block: one row per use-type per address, the enumerator's description of each establishment (`DSC_ESTABELECIMENTO`) and point; classified by `pipeline/taxonomies/brazil_cnefe.py` (free text, rules version 2) | All three buckets: **106,652** storefronts placed; unreadable descriptions dropped, never guessed; at an address that also holds a dwelling the pin shows its category only | `https://ftp.ibge.gov.br/Cadastro_Nacional_de_Enderecos_para_Fins_Estatisticos/Censo_Demografico_2022/Arquivos_CNEFE/CSV/Municipio/33_RJ/3304557_RIO_DE_JANEIRO.zip` (94,594,981 bytes) | none - the whole município file; `COD_ESPECIE` 6 read, 1-2 for the dwelling test, coordinate levels 1-4 kept | 2026-09-24 |
| Belo Horizonte | **IBGE CNEFE 2022** - the census's walk of every block: one row per use-type per address, the enumerator's description of each establishment (`DSC_ESTABELECIMENTO`) and point; classified by `pipeline/taxonomies/brazil_cnefe.py` (free text, rules version 2) | All three buckets: **45,582** storefronts placed; unreadable descriptions dropped, never guessed; at an address that also holds a dwelling the pin shows its category only | `https://ftp.ibge.gov.br/Cadastro_Nacional_de_Enderecos_para_Fins_Estatisticos/Censo_Demografico_2022/Arquivos_CNEFE/CSV/Municipio/31_MG/3106200_BELO_HORIZONTE.zip` (34,957,820 bytes) | none - the whole município file; `COD_ESPECIE` 6 read, 1-2 for the dwelling test, coordinate levels 1-4 kept | 2026-09-24 |
| Brasília | **IBGE CNEFE 2022** - the census's walk of every block: one row per use-type per address, the enumerator's description of each establishment (`DSC_ESTABELECIMENTO`) and point; classified by `pipeline/taxonomies/brazil_cnefe.py` (free text, rules version 2) | All three buckets: **36,687** storefronts placed; unreadable descriptions dropped, never guessed; at an address that also holds a dwelling the pin shows its category only | `https://ftp.ibge.gov.br/Cadastro_Nacional_de_Enderecos_para_Fins_Estatisticos/Censo_Demografico_2022/Arquivos_CNEFE/CSV/Municipio/53_DF/5300108_BRASILIA.zip` (19,814,371 bytes) | none - the whole município file; `COD_ESPECIE` 6 read, 1-2 for the dwelling test, coordinate levels 1-4 kept | 2026-09-24 |
| Salvador | **IBGE CNEFE 2022** - the census's walk of every block: one row per use-type per address, the enumerator's description of each establishment (`DSC_ESTABELECIMENTO`) and point; classified by `pipeline/taxonomies/brazil_cnefe.py` (free text, rules version 2) | All three buckets: **53,045** storefronts placed; unreadable descriptions dropped, never guessed; at an address that also holds a dwelling the pin shows its category only | `https://ftp.ibge.gov.br/Cadastro_Nacional_de_Enderecos_para_Fins_Estatisticos/Censo_Demografico_2022/Arquivos_CNEFE/CSV/Municipio/29_BA/2927408_SALVADOR.zip` (52,084,744 bytes) | none - the whole município file; `COD_ESPECIE` 6 read, 1-2 for the dwelling test, coordinate levels 1-4 kept | 2026-09-24 |
| Fortaleza (Regional) | **IBGE CNEFE 2022** - the census's walk of every block: one row per use-type per address, the enumerator's description of each establishment (`DSC_ESTABELECIMENTO`) and point; classified by `pipeline/taxonomies/brazil_cnefe.py` (free text, rules version 2) | All three buckets: **62,969** storefronts placed; unreadable descriptions dropped, never guessed; at an address that also holds a dwelling the pin shows its category only | `https://ftp.ibge.gov.br/Cadastro_Nacional_de_Enderecos_para_Fins_Estatisticos/Censo_Demografico_2022/Arquivos_CNEFE/CSV/Municipio/23_CE/2304400_FORTALEZA.zip` (28,907,115 bytes); `https://ftp.ibge.gov.br/Cadastro_Nacional_de_Enderecos_para_Fins_Estatisticos/Censo_Demografico_2022/Arquivos_CNEFE/CSV/Municipio/23_CE/2303709_CAUCAIA.zip` (3,875,954 bytes); `https://ftp.ibge.gov.br/Cadastro_Nacional_de_Enderecos_para_Fins_Estatisticos/Censo_Demografico_2022/Arquivos_CNEFE/CSV/Municipio/23_CE/2307650_MARACANAU.zip` (2,340,410 bytes); `https://ftp.ibge.gov.br/Cadastro_Nacional_de_Enderecos_para_Fins_Estatisticos/Censo_Demografico_2022/Arquivos_CNEFE/CSV/Municipio/23_CE/2309706_PACATUBA.zip` (710,306 bytes) | none - the whole município file; `COD_ESPECIE` 6 read, 1-2 for the dwelling test, coordinate levels 1-4 kept | 2026-09-24 |
| Porto Alegre (Regional) | **IBGE CNEFE 2022** - the census's walk of every block: one row per use-type per address, the enumerator's description of each establishment (`DSC_ESTABELECIMENTO`) and point; classified by `pipeline/taxonomies/brazil_cnefe.py` (free text, rules version 2) | All three buckets: **36,480** storefronts placed; unreadable descriptions dropped, never guessed; at an address that also holds a dwelling the pin shows its category only | `https://ftp.ibge.gov.br/Cadastro_Nacional_de_Enderecos_para_Fins_Estatisticos/Censo_Demografico_2022/Arquivos_CNEFE/CSV/Municipio/43_RS/4314902_PORTO_ALEGRE.zip` (17,172,345 bytes); `https://ftp.ibge.gov.br/Cadastro_Nacional_de_Enderecos_para_Fins_Estatisticos/Censo_Demografico_2022/Arquivos_CNEFE/CSV/Municipio/43_RS/4304606_CANOAS.zip` (3,546,253 bytes); `https://ftp.ibge.gov.br/Cadastro_Nacional_de_Enderecos_para_Fins_Estatisticos/Censo_Demografico_2022/Arquivos_CNEFE/CSV/Municipio/43_RS/4307708_ESTEIO.zip` (744,631 bytes); `https://ftp.ibge.gov.br/Cadastro_Nacional_de_Enderecos_para_Fins_Estatisticos/Censo_Demografico_2022/Arquivos_CNEFE/CSV/Municipio/43_RS/4320008_SAPUCAIA_DO_SUL.zip` (1,370,445 bytes); `https://ftp.ibge.gov.br/Cadastro_Nacional_de_Enderecos_para_Fins_Estatisticos/Censo_Demografico_2022/Arquivos_CNEFE/CSV/Municipio/43_RS/4318705_SAO_LEOPOLDO.zip` (2,436,268 bytes); `https://ftp.ibge.gov.br/Cadastro_Nacional_de_Enderecos_para_Fins_Estatisticos/Censo_Demografico_2022/Arquivos_CNEFE/CSV/Municipio/43_RS/4313409_NOVO_HAMBURGO.zip` (2,739,559 bytes) | none - the whole município file; `COD_ESPECIE` 6 read, 1-2 for the dwelling test, coordinate levels 1-4 kept | 2026-09-24 |
| Recife (Regional) | **IBGE CNEFE 2022** - the census's walk of every block: one row per use-type per address, the enumerator's description of each establishment (`DSC_ESTABELECIMENTO`) and point; classified by `pipeline/taxonomies/brazil_cnefe.py` (free text, rules version 2) | All three buckets: **44,382** storefronts placed; unreadable descriptions dropped, never guessed; at an address that also holds a dwelling the pin shows its category only | `https://ftp.ibge.gov.br/Cadastro_Nacional_de_Enderecos_para_Fins_Estatisticos/Censo_Demografico_2022/Arquivos_CNEFE/CSV/Municipio/26_PE/2611606_RECIFE.zip` (17,729,524 bytes); `https://ftp.ibge.gov.br/Cadastro_Nacional_de_Enderecos_para_Fins_Estatisticos/Censo_Demografico_2022/Arquivos_CNEFE/CSV/Municipio/26_PE/2607901_JABOATAO_DOS_GUARARAPES.zip` (7,528,738 bytes); `https://ftp.ibge.gov.br/Cadastro_Nacional_de_Enderecos_para_Fins_Estatisticos/Censo_Demografico_2022/Arquivos_CNEFE/CSV/Municipio/26_PE/2602902_CABO_DE_SANTO_AGOSTINHO.zip` (2,470,937 bytes); `https://ftp.ibge.gov.br/Cadastro_Nacional_de_Enderecos_para_Fins_Estatisticos/Censo_Demografico_2022/Arquivos_CNEFE/CSV/Municipio/26_PE/2603454_CAMARAGIBE.zip` (1,592,566 bytes) | none - the whole município file; `COD_ESPECIE` 6 read, 1-2 for the dwelling test, coordinate levels 1-4 kept | 2026-09-24 |
...
| Rennes | STAR (Keolis Rennes) — **Métro a and b** | `https://eu.ftp.opendatasoft.com/star/gtfs/GTFS_STAR_BUS_METRO_EN_COURS.zip` | 2026-09-23 | 13,119,510 bytes, no key and no account. Measured at 150 bus and **2 métro (`route_type 1`)**; STAR runs no tram. ⚠️ **`EN_COURS`, never `A_VENIR`** — the sibling resource is the FORTHCOMING timetable. ✅ **The feed self-attests**: `feed_info.txt` names Keolis Rennes, **2026-09-22 to 2026-10-18** — a four-week window, so `brief_check` will flag it early; that means refetch, not alarm. ⚠️ **This file host is not the portal's API.** `data.explore.star.fr` runs a DOMAIN-WIDE quota of 150,000 calls a day shared by every anonymous caller, and it was spent on 2026-09-23 (HTTP 429, errorcode 10003, reset 00:00 UTC) while this host kept answering. **Gate 3 exact on both lines against STAR's own layer**, `tco-metro-topologie-dessertes-td` (a 15=15, b 15=15), read after the quota reset on 2026-09-24; OpenStreetMap's route relations (`network=FR:STAR`) agree, and were the build-day count while the portal refused. The sibling `tco-metro-topologie-stations-td` carries `codeinseecommune` per station and puts the same four outside 35238 that step 1 finds geometrically. Licence **ODbL 1.0** (`odc-odbl`) — **notice 26 (STAR)**, which neither the OpenStreetMap notice nor Tisséo's discharges, because ODbL §4.3 requires the notice to name *which* database | 
| Oslo | Ruter via **Entur**, Norway's national access point — **T-bane 1–5 and trams 12, 13, 15, 17, 18, 19** | `https://storage.googleapis.com/marduk-production/outbound/gtfs/rb_rut-aggregated-gtfs.zip` | 2026-09-24 | 50,999,897 bytes, keyless. ⚠️ **HEAD is not a probe here** — Google Storage answers 200 with size 0; a ranged GET returns 206 and `PK`. ⚠️ **Every route type is EXTENDED** (`401` T-bane ×5, `902` tram ×6, `1008` ferry ×6, buses 7xx) — a basic-type reader sees no rail. ⚠️ **`feed_info.txt` names Entur and declares NO window**, so the fetch date pins the snapshot (Toulouse's case). Kommune only, trams drawn, ferries excluded — owner's calls; the 12 stations outside are all in Bærum. **Gate 3: 101 T-bane stations** (a secondary source, Norwegian Wikipedia); OSM's relations are too partial here (line 2: 25 stations vs the feed's 47). ⚠️ **The feed colours every T-bane line `EC700C` and every tram `0B91EF`**, so the per-line colours are Ruter's own as recorded on **OpenStreetMap**'s route relations (ODbL, notice 1), two lightened in hue to clear the colour check; tram 15 (line 12's route while the Briskeby line is closed) has none and takes this project's teal. Licence **NLOD** — **notice 29**, whose wording Entur specifies: "Data made available by Entur" + its logo (shown on the Oslo page, owner's call). Ruter's APP agreement bars copying "data som Ruter gjør tilgjengelig", but it is formed on installing the app and no data page incorporates it; read as governing the app, and recorded rather than buried | 
...
| Amsterdam | **GVB, via OVapi's national GTFS** — metro 50–54 (`route_type 1`) and 16 tram lines 1, 2, 4, 5, 6, 7, 12, 13, 14, 17, 19, 24, 25, 26, 27, 29 (`0`), `agency_id` GVB | `https://gtfs.openov.nl/gtfs-rt/gtfs-openov-nl.zip` (242,421,105 bytes), keyless | 2026-09-24 | **GVB publishes no feed of its own** (`gtfs.ovapi.nl/gvb/...` is a 404). **The CC0-labelled file, deliberately**: the README beside the usual `gtfs.ovapi.nl/nl/gtfs-nl.zip` grants only "You are free to use this data", while this one sits under the producer's `LICENSE.TXT` - "These works are available under CC0 1.0 Universal" - and is the same feed minus AVV and Thalys; GVB's data is CC0 upstream (DOVA's concession annex, both NDOV lokets). Choosing it AVOIDS the README question rather than answering it in the project's favour. `feed_info.txt` self-attests 20260923-20261212 and `fetch_sources.py` refuses an expired copy; OVapi asks every client to name itself in the User-Agent and accept gzip - the fetch does both. **A line's stations are its REGULAR route** - served on at least half the line's days in the window - because GVB runs up to 30 shapes per line here (short workings, weekend closures, works diversions); gate 3 exact on the metro against English Wikipedia (secondary): 20 / 19 / 8 / 14 / 15, 39 network-wide. Tram 3 is on GVB's map and runs no trip in the window - not drawn. **Tram colours are GVB's own**, read from `GVB_railnetwerk_31aug_2026.pdf` (linked from gvb.nl/plattegronden) as each line badge's fill - the feed carries none and OSM's are unusable; metro colours are the feed's `route_color`; five trams lightened or darkened in HSL lightness to clear the colour check (GVB gives 6 and 25 one red, 19 and 29 one orange). Licence **CC0** - **nothing to display**; MUST NOT say or imply the site represents GVB (README), no GVB marks (CC0 §4) |
...
| Copenhagen | **OpenStreetMap** — Metro **M1–M4** (`route=subway`, operators Inmetro / Metro Service A/S) and S-tog **A, B, Bx, C, E, F, H** (`route=light_rail`, operator DSB): 11 lines, **29 route relations** in the bbox before the whitelist | The three Overpass mirrors in `pipeline/osm.py`, bbox `55.40,12.00,56.00,12.70`, `out geom` on the relations and `node(r)` for their stops | 2026-09-24 | **Why not the agency: Rejseplanen's national GTFS exists, is keyless and current** (`https://www.rejseplanen.info/labs/GTFS.zip`, 54,971,494 bytes, modified 2026-09-21) **and was NOT downloaded**: its Labs guidelines grant CC BY 4.0 but ask users not to change the data so that it differs from the original, and document access as by request with those guidelines accepted. Neither point was resolved in this project's favour (owner's call 2026-09-24). ⚠️ **Every relation in the bbox carries `network=Takst Sjælland`** — the fare zone, Lokaltog and Movia's Letbane included — so the whitelist is **ref + route + operator**, never `network`. **S-tog is drawn on the published spacing-and-frequency test** (owner's call; Dublin's DART precedent): median in-scope spacing 1,227 m, every line every 10 minutes. Stations are the kept relations' `stop` members (all 283 are `public_transport=stop_position`), collapsed by name — eight names are Metro/S-tog interchanges, widest Nordhavn at 279 m. **Gate 3 exact**: Metro 15/16/17/13 = 44 (English Wikipedia's line table, secondary) and S-tog 87 (DSB's own page, which also says 86). Colours are the relations' own, S-tog A and F lightened against the shared ΔE floor. OpenStreetMap, ODbL 1.0 — notice 1 and the rail-geometry notice |
| Rome | **ATAC Metro A, B, B1 and C, and COTRAL's Roma–Viterbo urban service, from OpenStreetMap** — relations 207973/1720955 (A), 207926/1720959 (B), 2172804/2172805 (B1, tagged ref "B"), 398053/2172845 (C), 387417/1721478 (Roma–Viterbo, Flaminio–Montebello only) | The three Overpass mirrors in `pipeline/osm.py`, every `route=subway` and `route=light_rail` relation in the bbox 41.60,12.20,42.10,12.90, `out geom` | 2026-09-24 | **Why not the agency - the ground, put to the owner 2026-09-24.** Roma Mobilità's static GTFS (`https://romamobilita.it/sites/default/files/rome_static_gtfs.zip`, 46,798,074 bytes, no `feed_info.txt`) was downloaded and **NOT USED**: `licence-read` found it AMBIGUOUS IN A WAY THAT MATTERS - its own open-data page says the data *"possono essere utilizzati ... esclusivamente a titolo di supporto al viaggio"* (only as travel support) and reserves action against *"utilizzo improprio"*, and the one file carries five licence declarations (CC BY-SA with no version on the dataset page, *Licenza Sconosciuta* on the resource and in the catalogue record, CC BY 4.0 by harvest on dati.gov.it, CC BY 3.0 IT historically). Not resolved in the project's favour. RSM's own ArcGIS metro layers are CC BY 4.0 but last edited 2023-05-19 - Metro C stops at San Giovanni, missing Porta Metronia and Colosseo - Fori Imperiali (opened 2025-12-16). OSM carries all four lines to the current termini. Gate 3 exact against English Wikipedia (secondary): A 27, B 26 with B1, C 24, 74 network-wide; one alias (Colosseo / Colosseo - Fori Imperiali, one interchange). B1's relations run the whole trunk, so step 1 cuts them to B1's own ways for drawing. **COTRAL's two lines, measured against the DART / S-tog test (owner's call 2026-09-24)**: the Roma–Viterbo urban service is DRAWN - 15 stations, gate 3 exact against it.wikipedia (secondary), median 895 m apart, every 10–15 minutes, 14 of 15 with no metro within 800 m, Flaminio an interchange with Metro A; Metromare (Roma–Lido) is LEFT OUT - median 2,171 m apart and every 15–20 minutes in July 2026. Always out: OSM's "Metro D" (a line not yet built, no stops). OpenStreetMap, ODbL 1.0 — notice 1 |
...
| Salvador | **OpenStreetMap** - Metrô de Salvador Linha 1 and Linha 2 (CCR Metrô Bahia) | every rail route relation in the bbox | 2026-09-24 | OSM carries no colours; the operator's line NAMES (Vermelha, Azul) resolved through the CSS table. Aeroporto station is in Lauro de Freitas - excluded. OpenStreetMap, ODbL 1.0 - notice 1 |
...
| Oslo | **Kartverket kommuneinfo, kommune `0301`** — dataset *Administrative enheter kommuner* (uuid `041f1e6e-bdbc-4091-b48f-8a5990f3cc5b`) | `https://api.kartverket.no/kommuneinfo/v1/kommuner/0301/omrade` — Kartverket's recommended endpoint; the older `https://ws.geonorge.no/kommuneinfo/v1/kommuner/{code}/omrade` proxy (the build's first fetch, 2026-09-24) returns identical geometry for 0301 and 3201. Template: `https://api.kartverket.no/kommuneinfo/v1/kommuner/{code}/omrade` | the kommune alone — **480.5 km²** as measured (the polygon includes the inner fjord). **Kommune only, owner's call 2026-09-24**, on the French cities' rule: the worst line, T-bane 3, keeps 29 of 38 (76%), better than Toulouse's T1 and Rennes' Métro b. 167 stations network-wide → **155 inside, 12 outside**. `config.EXPECTED_INSIDE_PER_LINE` asserts the split. Licence **CC BY 4.0** — notice 28 |
| Oslo | **Bærum, kommune `3201`**, same endpoint — the NAMING layer, not the scoping one | `https://api.kartverket.no/kommuneinfo/v1/kommuner/3201/omrade` | not used to scope anything: it names the commune of the 12 excluded T-bane stations, Paris's naming layer in miniature. Step 1 exits if a station is outside Oslo AND outside every kommune listed in `config.NEIGHBOUR_KOMMUNER`. Same licence as the contour |
| Copenhagen | **OpenStreetMap kommune relations** (`boundary=administrative`, `admin_level=7`), polygonised from their outer AND inner member ways — Kobenhavn is relation 2192363, Frederiksberg 2186660 | The three Overpass mirrors in `pipeline/osm.py`, bbox `55.40,12.00,56.00,12.70`, `out geom` — 38 relations, 35 Danish | **Kobenhavn (`ref` 101) + Frederiksberg (147)**, owner's call 2026-09-24: **94.4 km²** and **8.7 km²**, gated at 80-110 and 8-10. ⚠️ **Frederiksberg is an enclave, so Kobenhavn's relation carries it as an INNER ring** — a polygon from outer ways alone would put Frederiksberg's stations in Kobenhavn; step 1 asserts the overlap is 0.000 km². Danish kommuner carry the kommunekode as `ref` (unpadded); Swedish ones in the same bbox carry `ref:scb` too and are skipped. DAWA's kommune polygons were used for the probe and are NOT used by the build (DAWA closes 1 October 2026). ⚠️ **The polygon decides only which STATIONS count** — businesses are scoped by CVR's own kommune code. 123 stations network-wide → **64 inside** (Kobenhavn 55, Frederiksberg 9), 59 outside, each named with its kommune from the same layer. OpenStreetMap, ODbL 1.0 — notice 1 |
| Prague | **OpenStreetMap relation 435514** ("Praha", `admin_level` 4, `ref:nuts` CZ010 - the region that is also the city), polygonised from its outer and inner ways | The three Overpass mirrors in `pipeline/osm.py`, `rel(435514);out geom;` | **Obec 554782 alone - 495.9 km²** against ČÚZK's INSPIRE polygon's 496.2, gated at 480-510. A bbox-bounded name search returns only this relation. **It scopes STATIONS only** - businesses are scoped by RUIAN's own address list for the obec - and all 58 metro stations are inside, so `outputs/prague/excluded_stations.csv` is written EMPTY. OpenStreetMap, ODbL 1.0 — notice 1 |
| Amsterdam | **OpenStreetMap relation 47811** ("Amsterdam", `admin_level` 8, `ref:gemeentecode` 0363, Weesp included since 2022), polygonised from its outer and inner ways; the 22 neighbouring gemeente relations, bbox-bounded, used ONLY to name excluded stations | The three Overpass mirrors in `pipeline/osm.py`: `rel(47811);out geom;` and `rel["boundary"="administrative"]["admin_level"="8"]["ref:gemeentecode"](52.18,4.70,52.47,5.15);out geom;` | **Gemeente 0363 alone - 243.6 km²** (CBS ~245), gated at 230-260; a bounded name search returns only this relation. Scopes STATIONS; both business layers are the gemeente's own and step 2 confirms every row inside. 22 stop places are outside - Amstelveen 12, Diemen 5, Uithoorn 3, Ouder-Amstel 2 - recorded in `outputs/amsterdam/excluded_stations.csv` with the 59 thinned tram stops. OpenStreetMap, ODbL 1.0 — notice 1 |
| Rome | **OpenStreetMap relation 41485** ("Roma", `admin_level` 8, `ref:ISTAT` 058091), polygonised from outer and inner ways (the Vatican is an inner ring); the comuni in the bbox, used ONLY to name an excluded station | The three Overpass mirrors: `rel(41485);out geom;` and `rel["boundary"="administrative"]["admin_level"="8"]["ref:ISTAT"](41.60,12.20,42.10,12.90);out geom;` | **Comune 058091 - 1,286.0 km²**, gated at 1,250-1,320; a bounded name search returns only this relation. Scopes STATIONS; the register is Roma Capitale's own. 73 of 74 stations inside - **Monte Compatri - Pantano**, Metro C's terminus, is in Monte Compatri (058060) - recorded in `outputs/rome/excluded_stations.csv`. OpenStreetMap, ODbL 1.0 — notice 1 |
...
| Riga's address points and degrading-buildings list (GEO RĪGA, Riga municipality) | Riga - a JOIN, not a geocoder: the excise register's addresses matched to `adreses.gpkg` (exact, then with the unit dropped; 96.8% placed), and shops in buildings listed in `vidi_degradejosas_buves.gpkg` left out | `https://data.gov.lv/dati/dataset/4e2bd2d1-69e1-4598-b2f0-713e963e55e3/resource/7fc97d43-2c34-4301-8776-a5682273115e/download/adreses.gpkg`; `https://data.gov.lv/dati/dataset/9789c471-5751-4b20-87ac-7aba43c42f8b/resource/b9521b89-d158-4884-8e6d-c60873cf5241/download/vidi_degradejosas_buves.gpkg` | CC BY 4.0 (read 2026-09-24); neither layer is shown. Notice 43, Riga municipality |
...
| Source | Verdict | What it requires |
|---|---|---|
| **MLIT 位置参照情報** (address points, block 24.0a and town-chōme 19.0b): the coordinate JOIN for every city | **PERMITTED WITH CONDITIONS — PDL 1.0**, through the site terms in force 2026-03-23 (`https://nlftp.mlit.go.jp/ksj/other/agreement.html`). The files carry no rights notice. PDL 1.0 allows adaptation and 「商用利用も可能です」. Read 2026-09-24 | **MUST DISPLAY** `出典：位置参照情報ダウンロードサービス（国土交通省）（https://nlftp.mlit.go.jp/isj/）` and `「位置参照情報ダウンロードサービス」（国土交通省）（https://nlftp.mlit.go.jp/isj/）を加工して作成`, naming 街区レベル and 大字・町丁目レベル位置参照情報. **MUST NOT** present the placements as MLIT's own |
| **MLIT N02 鉄道データ** (N02-24): stations and lines for every city | **PDL 1.0**, read 2026-09-21 | `「国土数値情報（鉄道データ）」（国土交通省）をもとに作成` |
| **MLIT N03 行政区域** (N03-2025; an N03-2026 edition now exists): the city line that decides which stations count | **PERMITTED WITH CONDITIONS, for the use made: picking stations and anchoring labels, with no N03 geometry drawn.** The N03-2025 page's own 使用許諾条件 is オープンデータ（CC_BY_4.0）, linking MLIT's terms: 「…自由に利用できます。商用利用も可能です。」 ⚠️ **But the same page's 原典表示 says the boundaries derive from GSI's 数値地図（国土基本情報）, under Survey Act (測量法) approval, and that reproducing them needs GSI's approval.** GSI's own guidance, applied to a public web map, ends at "application required". So **drawing the outline is AMBIGUOUS in a way that matters**, and is not resolved in this project's favour. This project's maps never draw a city boundary. The verdict holds only while that stays true. Read 2026-09-24 | **MUST DISPLAY**, per MLIT's template (examples, not fixed wording): `出典：国土交通省国土数値情報ダウンロードサイト（https://nlftp.mlit.go.jp/ksj/gml/datalist/KsjTmplt-N03-2025.html）` and `「国土数値情報（行政区域データ）」（国土交通省）（同URL）をもとに[作成者名]作成`, plus the licence link if relying on CC BY. **MUST NOT**: present it as MLIT's own; call the boundaries definitive (some are provisional, 暫定). ⛔ **MUST NOT DRAW N03 geometry on any map** until GSI approves (application through the 測量成果ワンストップサービス, which needs an account, so it is the owner's act) or MLIT answers (`shitsumon@ksjask.info`). The same rule is in `pipeline/countries/japan.py`. Aside: MLIT's current examples add a URL and the processor, which the N02 credit above lacks. Check at build |
| **MHLW 食品衛生申請等システム open data**: Fukuoka's and Hiroshima's online filings, and a slice in every city | **PERMITTED WITH CONDITIONS — PDL 1.0**, from the system's site terms (`https://i2fas.mhlw.go.jp/termsofuse.htm` §2). The system's own 利用規約 binds applicants and authorities only (第3条), so its 第7条 ban on alteration does not reach reusers. Archived: `docs/licenses/mhlw-food-sanitation-system-terms-2020.pdf`, and a 2026 amendment that leaves those clauses untouched | **MUST DISPLAY** the source, that it was processed, and by whom (重要情報 1.1). Link the top page only. **MUST NOT**: present it as MHLW's own; use MHLW's logo; claim completeness (the data is opt-in) or accuracy. Cost: 免責 1) エ, ✅ **ACCEPTED** with every Japanese source. ⚠️ **OPEN (minor)**: 免責 2) ウ could be read as barring commercial reuse. That is fine while the site is non-commercial |
| **Tokyo: the Tokyo catalogue's ward files**. Food: Chūō, Minato, Shinjuku, Kōtō. 生活衛生 registers: 11 wards | **PERMITTED WITH CONDITIONS**: Tokyo Open Data Terms §2 (「商用利用も可能です」), and the catalogue declares CC-BY-4.0 on every package used. Shinjuku's own site offers the same file under CC BY 2.1 JP. Read 2026-09-24 | **One combined notice**: ward, the Tokyo catalogue, dataset title, URL, **date of use**, that the data was processed, and the CC BY 4.0 link. **MUST NOT** present anything as the ward's or Tokyo's own, or use a ward logo. Cost: Tokyo §6 / Chūō §5. ✅ **ACCEPTED by the owner 2026-09-24** |
| **Tokyo: Shibuya**: `131130_food_businesses_list.csv` on the ward's ArcGIS open-data site (item `e68f41ebfa5f4ea490ca9af701d44e02`) | **PERMITTED WITH CONDITIONS**: 渋谷区オープンデータ利用規約 (2018-11-07, built on 政府標準利用規約 2.0). The site's home page says downloading is accepting it. §1 grants copying, transmission and adaptation, commercial use included. §6(3) makes it compatible with CC BY 4.0, and the item declares CC BY. Read 2026-09-24 | **MUST DISPLAY** (the §2 pattern for modified use): `「食品等営業許可・届出一覧」（渋谷区）（https://city-shibuya-data.opendata.arcgis.com/datasets/e68f41ebfa5f4ea490ca9af701d44e02_0/about）を加工して作成`, plus CC BY 4.0 linked if relying on that route. **MUST NOT**: present it as the ward's own; imply endorsement. **No cost clause** on the user. The ward's dashboard has a broader 補償 clause, but it governs the dashboard only. The terms may change without notice, so re-read them before each republish. Contact for removals: div-smartcity@shibuya.tokyo |
| **Tokyo: Taitō**: `2026ALL-IND-CSV.csv`, linked from the ward's food page (`…/food/syokuhin-sisetu/index.html`) | **PERMITTED WITH CONDITIONS — CC BY 4.0**. The site policy bars copying, but the ward's licence page (`/kusei/online/opendata/about/opd.html`) puts everything on its オープンデータ一覧 under CC BY 4.0, and this list is on it (catalogue entry, metadata CSV row 34). It is also on the Tokyo catalogue (`t131067d3100000006`, a link-only entry), so the Tokyo Open Data Terms apply too. The two agree. Read 2026-09-24 | **MUST DISPLAY** the ward's four elements: `台東区`; `CC-BY表示4.0国際` (linked); `本作品の内容について、台東区は一切保証しないものとする。`; and the page URL. Also add that the data was modified (CC §3(a)(1)(B), Tokyo §2(1)イ). A link must say it goes to 台東区公式ホームページ, with no framing. **MUST NOT**: imply endorsement or authorship; use logos. Cost: Tokyo §6, the accepted class. Optional courtesy: a note to 情報政策課 about the use (not a term). The ward masks individual operators at source (※, 2,738 of 7,676 rows) |
| **Tokyo: Setagaya**: `zenkenr080331.csv` and the monthly files on page 3246 (`/02245/online_tetsuzuki/3246.html`) | **PERMITTED WITH CONDITIONS — CC BY 4.0**, through 世田谷区オープンデータ利用規約 (`/01000/5451.html`) §2. The page says the files are open data, gives each a CC BY 4.0 badge, and says using them accepts the terms. The copyright page's default bar exempts open data by its own wording. **Not on the Tokyo catalogue**, so Tokyo's terms and date-of-use notice do not apply. Read 2026-09-24 | **MUST DISPLAY** the §2 form for modified use: `この[地図]は以下の著作物を改変して利用しています。[データ名称]、世田谷区、クリエイティブ・コモンズ・ライセンス 表示4.0国際（https://creativecommons.org/licenses/by/4.0/deed.ja）`. The name is the file's label (`全件許可施設一覧(R080331)`) or the page title; choose at build. Add the page URL. **MUST NOT**: present it as the ward's own; imply endorsement; use the emblem or the オープンデータせたがや mark. Cost: §4, identical in effect to Tokyo §6, the accepted class. The ward masks individual operators at source (***) |
| **Tokyo: Meguro**: food lists on BODIK (`dbbf7d8e-519d-4ce2-b789-f122b7003644`, `all_new_8.csv` / `all_old_8.csv`), and its barber, beauty and laundry registers (`131105_barber`, `131105_hairdressingshop`, `131105_cleaning_shop`, as of 2026-03-31) | **PERMITTED WITH CONDITIONS — CC BY 4.0** (`cc-by-40-intl` on all 171 of the ward's packages). Terms: `https://odcs.bodik.jp/131105/tos/` (2021-08-30, the Tokyo template with 目黒区 swapped in). Use is acceptance (第1項), and the terms override the ward's other site terms (第5項). Read 2026-09-24 | **MUST DISPLAY**: each title used (the resource names carry the as-of date), 目黒区, CC BY 4.0 linked, a statement that the map modifies them, and the catalogue link labelled 目黒区オープンデータカタログサイト (第9項). No date of use. **MUST NOT**: present it as the ward's or any Tokyo municipality's own; use logos alone (第2項(3)); frame the catalogue. Cost: 第8項, character-identical to Tokyo §6, the accepted class. The food lists carry 営業者氏名 (never read) |
| **Tokyo: Shinjuku's 2026-03-31 PDF list** (「令和7年度末全食品衛生許可施設一覧」, `/content/000452833.pdf`) and its monthly PDFs | 🚫 **NOT PERMITTED, or ambiguous in a way that matters** (read 2026-09-24). The site terms (`/kusei/about.html`) name PDFs and forbid 二次利用 and modification without prior permission, with no exception. The open-data terms reach only the portal, which sends all else back to the site terms. Two readings would permit it: the ward treats the list as open data (its application form says details are published as open data "in principle"), or the premises data are uncopyrightable facts. The ward's own documents make neither for this list. **Not relied on; not used.** Shinjuku stays on its 2023 CSV | Under any reading, the linking rule applies: say a link goes to the Shinjuku official site, and **never link a PDF directly**. Cite `eisei04.html` |
| **Tokyo: Chūō's 環境衛生営業施設一覧**: 理容所, 美容所 and クリーニング所 xlsx as of 2026-08-31 (`/documents/5654/`, linked from `…/kankyoueisei/kankyoueiseisisetsuichiran.html`) | 🚫 **NOT PERMITTED, or ambiguous in a way that matters** (read 2026-09-24). The lists are not on the ward's open-data page (`opendata.html`, whose index `open_data_list.csv` has 36 files) or on the Tokyo catalogue (organisation `t131024`: 47 packages, none of these). The open-data page opens 「当ライセンスは、本サイトに掲載しているデータのみに適用されます」 and sends other data to the site's 著作権について (`thissite.html`). That clause names 「PDFその他のデータ」 and says 「著作権法上認められている行為を除き、無断で転載や改変などを行うことはできません」. Two readings would permit it: guideline 第２章1 (「原則として、本区ホームページで公開しているデータを対象とする」, a policy on what to open, not a grant to users), or premises facts are not 著作物 (declined for Sendai and Shinjuku). **Not relied on; not used.** The §5 cost clause does not reach it | Nothing owed while unused. Any link must say it goes to the 中央区ホームページ. The list page warns entries may be out of date (closed premises): never say "currently operating". Permission channel: 保健所生活衛生課環境衛生担当, phone 03-3541-5938 and fax only. **Contacting it is the owner's last resort** |
| **Tokyo: Shinjuku's 生活衛生 premises lists**: 理容所, 美容所, クリーニング所 and 旅館業 PDFs as of 2026-06-30 (`/content/000452993.pdf` to `000452996.pdf`), plus the monthly 新規 PDFs, on `eisei03_002069.html` | 🚫 **NOT PERMITTED, or ambiguous in a way that matters** (read 2026-09-24). The same site terms as the food PDF: `/kusei/about.html` names PDFs and other data, and forbids 二次利用, modification and redistribution without prior permission. **No open-data version exists** anywhere: not on the ward's own list (142 rows), the Tokyo catalogue (organisation `t131041`: 137 packages), LinkData (98 works) or the 2021 archived page. There is no mark on the page or the PDFs, which are titled 情報公開リスト. Unlike food, the opening forms say nothing about open data, so the only permitting reading left is that premises facts are not 著作物. **Not relied on; not used** | Under any reading: say a link goes to the Shinjuku official site, never link a PDF, cite `eisei03_002069.html`. Use needs prior permission from 衛生課 環境衛生係 (03-5273-3841) or 区政情報課: the owner's act and last resort. ⚠️ The PDFs carry 営業者名, 代表者名, **営業者住所** and 施設TEL unmasked: never read |
| **Tokyo 総務局's COVID-era lists**: 徹底点検TOKYOサポート 認証店 (`soumu/130001_tokyo_tenken.csv`) and 感染防止徹底宣言ステッカー (`soumu/bousai/130001_tokyo_tenpomap.csv`), frozen May 2023 | 🚫 **NOT USED (owner, 2026-09-24)**, honouring the stated purpose. Ambiguous in a way that matters (not given a full `licence-read`). The catalogue declares CC BY 4.0, but each dataset's note says the shops agreed to share their data for COVID measures and asks users not to use it for any other purpose (「それ以外の目的に用いないようにお願いします」). CC BY cannot carry such a condition, but the publisher's stated wish and the shops' consent are both limited to that purpose, and this project honours a publisher's request rather than arguing it | They would lift Chūō, Kōtō and Minato to about 50–60% of the official count |
| **OpenStreetMap as a food-premises source** (Tokyo) | **Measured, not used** (2026-09-24). ODbL 1.0, so a rendered map is a Produced Work, but committed points are a Derivative Database under share-alike. It holds only 8–10% of Shibuya's registered restaurants and is systematically biased toward chains and street-level cafés | — |
| **Osaka City**: food permits and the barber, beauty and laundry registers | **PERMITTED WITH CONDITIONS**: the city's 著作権・免責 page is 政府標準利用規約 2.0, usable equally under CC BY 4.0. Each source page says 「CC-BY4.0で提供いたします。」 Read 2026-09-24 | **MUST DISPLAY**, per page: `「食品営業許可施設一覧」（大阪市）（https://www.city.osaka.lg.jp/kenko/page/0000575579.html）を加工して作成`, likewise pages 0000431136 and 0000552712. **MUST NOT** present the map as the city's own, or use city logos. ✅ **ACCEPTED by the owner**: the current food CSV's licence rests on the CC BY notice on the page that links it |
...
**Fault-based cost clauses. ✅ ACCEPTED by the owner 2026-09-24, for all of
Japan, as for Taiwan.**
- **What they say**: several Japanese terms make the user bear the cost of
  claims arising from its own breach. They are uncapped but fault-based, the
  class of Taiwan's OGDL §六(三), Rio's and IBGE's, and narrower than Hong
  Kong's indemnity.
- **The decision**: one acceptance covers every Japanese source with a clause
  of this class. That is Tokyo §6 / Chūō §5 and Sapporo 第9条3 (each first
  accepted on its own), Fukuoka 第４条, Hiroshima 第3条 and MHLW 免責 1) エ, and any
  Japanese source read later whose clause is of the same class.
- **Its limit**: a clause that goes further, such as an indemnity for claims
  the project did not cause, or a capped or strict liability, is a new
  question for the owner.

**What may not be said.** The Kobe, Sapporo and Kyoto datasets each warn that
listed premises may have closed, so no Japanese page says "currently
