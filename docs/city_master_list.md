...
| **Kuala Lumpur 🇲🇾** ▼ *from the discards 2026-09-24* | DOSM's `lookup_premise`: 372 KL rows, a price-survey frame, not a register — national only | DBKL's own catalogue for a licensing register ▶ **Re-probed 2026-09-24 — NEGATIVE on two methods, the city's own host asked.** DBKL's open-data page enumerated (10 datasets, no licence register); its ArcGIS server enumerated — two folders token-gated, and `Hosted/LESEN_AKTIF` is **102 rows** beside 42 CCTV points and 73 lamps: a dashboard mock-up, not a register. data.gov.my: 292 datasets, none from any local authority. **RECOMMEND (owner): to the discard table** (kind absence; `check_discard_evidence.py` passes it) |
...
| **Warsaw 🇵🇱** ▼ *from the discards 2026-09-24* | `dane.gov.pl` (control passes): CEIDG is company-level; one alcohol-sales-points lead, one bucket at best — national only | `api.um.warszawa.pl` for a municipal premises dataset ▶ **Re-probed 2026-09-24 — STILL THIN; the city's own catalogue is GEO-BLOCKED.** `api.um.warszawa.pl` / `dane.um.warszawa.pl` answer only from Poland (2/2), time out from Germany and the US, while `mapa.um.warszawa.pl` answers everywhere — a block, not an outage (and the API is key-gated). The alcohol-sales-points lead was **Gdańsk's**, not Warsaw's. Reachable routes negative: the city's 21 datasets on dane.gov.pl; the map portal's layers (markets, café terraces only). **RECOMMEND (owner): Band D** (access blocked by the publisher). Side find: Gdańsk publishes licensed alcohol sales points (CC BY 4.0, 2026-09-09) — unmeasured |
| **Hamburg 🇩🇪** ▼ *from the discards 2026-09-24* | `suche.transparenz.hamburg.de`, ONE term (*Gewerberegister*) → 7 irrelevant hits, no control | Enumerate the transparency portal and geoportal (Gaststätten, Einzelhandel) with a nonsense-term control ▶ **Re-probed 2026-09-24 — PARTIAL.** The city's own API (`api.hamburg.de/datasets/v1/einzelhandel` + `einzelhandel_o_nv`): a **2016 on-site survey of every retail business — 9,916 points** with address, 38 sector groups, business type, sales area; dl-de-by-2.0 declared; **1.17× OSM retail**. Retail ONLY and ten years old; no food or personal-services register in either catalogue (transparency portal enumerated, 521 map-service titles read; the city API's 411 datasets read). ALKIS building functions exist but are whole buildings. **Owner's call: Band C shape (a stale one-bucket retail census) or discard on currency** |
...
| **Athens 🇬🇷** ▼ *from the discards 2026-09-24* | `data.gov.gr`, 343 hits, sector-specific (shipyards, wineries) — the top hits of one search | The municipality's own portal and `geodata.gov.gr` for shop or food licences ▶ **Re-probed 2026-09-24 — NEGATIVE on two methods, the city's own host asked.** The city's CKAN (`opendata.cityofathens.gr`, nonsense 0): 464 datasets, none premises; its GeoServer: 60 layers, none. data.gov.gr (now CKAN) enumerated — 22,591 datasets; the national business-notification list is 72,378 rows from 2018 with no address or municipality. `geodata.gov.gr` is dead (times out from GR and DE); `www.cityofathens.gr` is behind a Cloudflare challenge (not bypassed). **RECOMMEND (owner): to the discard table** |
| **Poznań 🇵🇱** ▼ *from the discards 2026-09-24* | `dane.gov.pl`: statistical hits; CEIDG and REGON keyed — national only | Poznań's own portal and GIS for premises ▶ **Re-probed 2026-09-24 — NEGATIVE on two methods, the city's own host asked.** The city portal (`www.poznan.pl/opendata`, GraphQL — read-only POST queries, since it refuses GET; nonsense 0): 66 datasets, none premises; the city's 35 datasets on dane.gov.pl; its feature server (35 layers), map POIs (156 categories) and GIS portal (30 services) — none. Alcohol permits are published only as aggregates (e.g. 840 category-A food-service permits). **RECOMMEND (owner): to the discard table.** Side find: the city portal carries a **tram GTFS modified 2026-09-18** — this list's rail row (expired 2026-06-05) is stale |
| **Bratislava 🇸🇰** ▼ *from the discards 2026-09-24* | The national RPO API: addressed, but no activity classification — national only | Bratislava's own open-data portal for premises or permits ▶ **Re-probed 2026-09-24 — NEGATIVE on two methods, the city's own host asked.** The city's ArcGIS Hub `data.bratislava.sk` (680 items), its ArcGIS Online org (2,093) and the geoportal's server (724 of 1,023 services opened; 299 accented names judged by name) — no premises, food-service or opening-hours register. The only use-class layer is a national 1.7M-point address layer whose category is the building's and partly OSM-sourced. data.slovensko.sk (SPARQL) enumerated — 22,559 datasets, building-level only. **RECOMMEND (owner): to the discard table** |
