...
| 4 | Chicago | CTA 'L' (145 stations, 8 lines) + Metra | Business Licenses (Socrata `r5kz-chrr`): address, `latitude`/`longitude` | `chicago_license` (skeleton) | **Probed live 2026-09-19 (see `DECISIONS.md`).** No NAICS field. 150 `license_description` values, but the largest active bucket ("Limited Business License") is a catch-all, so the mapping also needs `business_activity` (4,120 values). The dataset holds every license term since 2002: filter to currently active licenses and dedupe by account and site. Metra's feed is at `schedules.metrarail.com` |
