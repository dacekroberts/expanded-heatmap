"""Taoyuan heatmap page - embeds the pre-rendered Folium HTML.

Same static-HTML-embed pattern as pages/1_San_Diego_Heatmap.py - see
Overview.py's docstring for why this is the decided pattern
for every city's detail page. Taichung's page is its template.
"""
...
st.set_page_config(page_title="Taoyuan Heatmap", page_icon="\U0001f5fa️", layout="wide")
set_base_font()
...
if PROVENANCE_JSON.exists():
    try:
        ...
        if _plates:
            _bits.append(f"door plates from **{_plates}** (Taoyuan City Government)")
        if _osm:
            _bits.append(f"the line's route as mapped in OpenStreetMap on **{_osm}**")
        if _bits:
