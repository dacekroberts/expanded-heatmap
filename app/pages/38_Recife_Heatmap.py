"""Recife (Regional) heatmap page - embeds the pre-rendered Folium HTML.
...
st.set_page_config(page_title="Recife (Regional) Heatmap", page_icon="\U0001f5fa\ufe0f", layout="wide")
set_base_font()

render_city_nav("Recife (Regional)")

st.title("Recife: commercial density around metro stations")
...
colours OpenStreetMap records. The two diesel VLT lines, to Curado and to Cabo, are not
drawn: suburban railways are left out of these maps unless they run like a metro, and
their stations are 3 to 4 km apart.
...
**About a quarter of the storefronts sit within a station ring.**
...
# The snapshot dates, read from outputs/recife/provenance.json so they cannot
...
if HEATMAP_HTML.exists():
    ...
    st.info("No map yet. Run `python pipeline/recife/step3_map.py` to generate it.")
