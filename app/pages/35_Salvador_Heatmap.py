"""Salvador heatmap page - embeds the pre-rendered Folium HTML.
...
from pipeline.salvador.config import HEATMAP_HTML, PROVENANCE_JSON  # noqa: E402
...
st.set_page_config(page_title="Salvador Heatmap", page_icon="\U0001f5fa\ufe0f", layout="wide")
set_base_font()

render_city_nav("Salvador")

st.title("Salvador: commercial density around metro stations")

# Approved by the owner 2026-09-24, with the Brazil batch's rail and scope
# calls applied as recommended.
st.markdown(
    """
Two lines are drawn — **Linha 1-Vermelha and Linha 2-Azul** — each labelled on the map
and in the legend. The lines and stations are OpenStreetMap's, which records no colours
for them, so each is drawn in the colour the operator names it by, red and blue. The map
covers the **município of Salvador**; Linha 2's last station, Aeroporto, lies just over the
boundary in Lauro de Freitas and is not counted.
...
# The snapshot dates, read from outputs/salvador/provenance.json so they cannot
...
if HEATMAP_HTML.exists():
    ...
    st.info("No map yet. Run `python pipeline/salvador/step3_map.py` to generate it.")
